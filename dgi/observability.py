...
class StructuredLogger:
    ...
    def _get_context_extras(self, extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for name, business_metric in self._business_metrics.items():
            if business_metric.threshold_warning or business_metric.threshold_critical:
                ...
                if current_value is not None:
                    if (business_metric.threshold_critical and 
                        current_value >= business_metric.threshold_critical):
                        alerts.append({
                            "metric": name,
                            "level": "critical",
                            "value": current_value,
                    ...
                    elif (business_metric.threshold_warning and 
                          current_value >= business_metric.threshold_warning):
                        alerts.append({
                            "metric": name,
                            "level": "warning", 
                            "value": current_value,
    ...
    def monitor_operation(self, operation_name: str, **metadata):
        ...
        try:
            ...
            self.metrics.increment_counter(
                ...
                labels={"status": "success", "user_type": context.user_id or "anonymous"}
            )
...
class ObservabilityManager:
    ...
    @contextmanager
    def request_context(self, correlation_id: Optional[str] = None, user_id: Optional[str] = None, request_id: Optional[str] = None):
...
def track_business_operation(operation_name: str, **metadata):
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs):
            with observability_manager.performance.monitor_operation(operation_name, **metadata):
...
def track_user_action(action: str, user_id: Optional[str] = None, **metadata):
